        ...
        if JavaInfo in dep:
            ...
            if not dep.label.workspace_name:
                ...
                if clean_label.startswith("@@//"):
                    clean_label = clean_label[2:]  # @@//pkg:lib -> //pkg:lib
                elif clean_label.startswith("@//"):
                    clean_label = clean_label[1:]  # @//pkg:lib -> //pkg:lib
                targets.append(clean_label)
    return targets

def _collect_classes_output_dirs(deps):
    """Derives bazel-bin output paths for .class files from deps.

    For each dep with JavaInfo, inspects JavaInfo.outputs.jars to find
    the class jar path. In Bazel, java_library produces a -class.jar
    containing compiled .class files.
...
def _quarkus_dev_impl(ctx):
    ...
    file_watcher_jar = ctx.file.file_watcher_tool
    ...
    classes_output_dirs = _collect_classes_output_dirs(ctx.attr.deps)
    ...
    ctx.actions.expand_template(
        ...
        substitutions = {
            ...
            "%{file_watcher_jar}": file_watcher_jar.short_path,
        },
        is_executable = True,
    )
...
quarkus_dev_rule = rule(
    ...
    attrs = {
        ...
        "file_watcher_tool": attr.label(
            allow_single_file = [".jar"],
            doc = "Java file watcher deploy jar for hot-reload (set by macro).",
        ),
    },
)
