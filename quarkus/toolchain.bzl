...
def _quarkus_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "QUARKUS_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    quarkusinfo = QuarkusInfo(
...
quarkus_toolchain = rule(
    implementation = _quarkus_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a quarkus compiler/runtime toolchain.
