...
class VersionCheckerTest {
  ...
  void matchingVersions_noWarnings() {
    ...
    List<ExtensionInfo> mismatched =
        VersionChecker.check(
            List.of(ext("quarkus-arc", "3.27.4"), ext("quarkus-resteasy-reactive", "3.27.4")),
            "3.27.4",
            capture.stream);
  ...
  void mixedVersions_onlyMismatchesReported() {
    ...
    List<ExtensionInfo> mismatched =
        VersionChecker.check(
            List.of(ext("quarkus-arc", "3.27.4"), ext("quarkus-resteasy-reactive", "3.18.0")),
            "3.27.4",
            capture.stream);
  ...
  void emptyExtensionList() {
    var capture = new CapturedOutput();
    assertEquals(0, VersionChecker.check(List.of(), "3.27.4", capture.stream).size());
    assertEquals("", capture.text());
  }
