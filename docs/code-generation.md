...
by `bazel test`. When `quarkus_app(continuous_test = ":test")` references the
test target, the running dev session also watches its declared test generator
inputs, rebuilds through Bazel, and notifies continuous testing after syncing
the outputs. This includes inputs whose edits leave generated bytecode unchanged.

`QuarkusCodeGenInfo` exposes the exact declared generator inputs as
`input_files`. It replaces the former `input_dirs` field: watching directories
rebuilt on unrelated files, and new files matching a glob are now discovered
from the declared inputs' roots instead.
